...
"""
    ...
    # Generate Plants
    # Rows at y = 1.0, 0.0, -1.0 (but robot needs to navigate between them)
    # The previous file had rows at y=1 and y=-1.
    # The image shows the robot straddling a row, or between rows.
    # Let's create rows at y = -1.5, -0.5, 0.5, 1.5
    # Plants spaced by 0.5m
    ...
    while current_x <= end_x:
        for y in rows_y:
            ...
            if is_healthy:
                stem_color = "0.1 0.6 0.1 1" # Green
                leaf_color = "0.2 0.8 0.2 1" # Bright Green
            else:
    ...
    <model name='plant_{plant_count}'>
      <pose>{current_x} {y} 0.1 0 0 0</pose>
      <static>true</static>
      <link name='link'>
        <visual name='visual'>
          <geometry>
            <!-- A simpler plant representation: a green sphere on a small cylinder -->
            <cylinder>
                <radius>0.08</radius>
                <length>0.2</length>
            </cylinder>
